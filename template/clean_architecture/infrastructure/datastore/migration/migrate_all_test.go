...
func connectDB() *gorm.DB {
	env.LoadConf("/Users/primawinangun/Documents/aksara/barber-backend/config.yaml")
	m := env.ENV.Pgsql
...
func TestMigrateAll(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := MigrateAll(tt.args.db); (err != nil) != tt.wantErr {
				t.Errorf("MigrateAll() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
